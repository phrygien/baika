        ...
        <form wire:submit="save">
            <div class="space-y-6">
                ...
                <div class="flex items-center gap-4 pr-8">
                    <div class="flex size-12 shrink-0 items-center justify-center rounded-xl border border-zinc-200 bg-zinc-50 text-2xl dark:border-zinc-700 dark:bg-zinc-800">
                        @if ($icon)
                            {{ $icon }}
                        @elseif ($image)
                            <img src="{{ $image }}" alt="" class="size-full rounded-xl object-cover" onerror="this.style.display='none'" />
                        @else
                    ...
                    <div class="space-y-4 p-4">
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <flux:input
                                ...
                                label="{{ __('Slug') }}"
                                placeholder="electronics"
                                description="{{ __('Auto-generated from name') }}"
                            />
                        </div>
                    ...
                    <div class="flex items-center justify-between border-b border-zinc-100 px-4 py-3 dark:border-zinc-800">
                        ...
                        <div class="flex items-center gap-2">
                            @if ($this->selectedParent && !$showChangeParent)
                                <button
                                    type="button"
                                    x-on:click="$wire.set('showChangeParent', true)"
                                    class="flex items-center gap-1 rounded-md px-1.5 py-0.5 text-xs text-zinc-400 transition hover:bg-zinc-100 hover:text-zinc-600 dark:hover:bg-zinc-800"
                                >
                                ...
                                <button
                                    ...
                                    class="flex items-center gap-1 rounded-md px-1.5 py-0.5 text-xs text-red-400 transition hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-950/20"
                                >
                            ...
                            @elseif ($showChangeParent)
                                <button
                                    type="button"
                                    x-on:click="$wire.set('showChangeParent', false); $wire.set('parentSearch', '')"
                                    class="flex items-center gap-1 rounded-md px-1.5 py-0.5 text-xs text-zinc-400 transition hover:bg-zinc-100 hover:text-zinc-600 dark:hover:bg-zinc-800"
                                >
                                    <flux:icon name="x-mark" class="size-3" />
                                    {{ __('Cancel') }}
                    ...
                    <div class="p-4 space-y-2">
                        ...
                        @if ($this->selectedParent && !$showChangeParent)
                            <div class="flex items-center gap-3 rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 dark:border-blue-800 dark:bg-blue-950/20">
                                <div class="flex size-8 items-center justify-center rounded-lg bg-white text-lg shadow-sm dark:bg-zinc-800">
                                    {{ $this->selectedParent->icon ?? '📁' }}
                                </div>
                                <div class="min-w-0 flex-1">
                                    ...
                                    <p class="text-xs text-zinc-400">
                                        {{ __('Depth') }} {{ $this->selectedParent->depth ?? 0 }}
                                        · {{ $this->selectedParent->slug }}
                                    </p>
                                </div>
                                <flux:badge size="sm" color="blue" inset="top bottom">
                                    {{ __('Level') }} {{ ($this->selectedParent->depth ?? 0) + 1 }}
                                </flux:badge>
                            </div>

                        {{-- Formulaire changement parent --}}
                        @elseif ($showChangeParent || !$this->selectedParent)
                            <flux:input
                                wire:model.live.debounce.200ms="parentSearch"
                                icon="magnifying-glass"
                                placeholder="{{ __('Search parent category...') }}"
                                size="sm"
                            />
                            ...
                            @if (strlen($parentSearch) >= 1)
                                <div class="max-h-40 overflow-y-auto rounded-lg border border-zinc-200 dark:border-zinc-700">
                                    @forelse ($this->parentResults as $cat)
                                        ...
                                        >
                                            <div class="flex size-7 shrink-0 items-center justify-center rounded-md bg-zinc-100 text-base dark:bg-zinc-800">
                                                {{ $cat->icon ?? '📁' }}
                                            </div>
                                            <div class="min-w-0 flex-1">
                                                <p class="text-sm font-medium text-zinc-800 dark:text-zinc-200">
                            ...
                            @else
                                <p class="text-xs text-zinc-400">
                                    {{ __('Search to select a new parent, or leave empty for root.') }}
                                </p>
                            @endif
                        @endif
                    ...
                    <div class="space-y-4 p-4">
                        <div class="grid grid-cols-2 gap-4">
                            <flux:input
                                ...
                                max="100"
                                step="0.5"
                                description="{{ __('Optional, overrides parent (%)') }}"
                            />
                            <flux:input
                ...
                <div class="rounded-xl border border-zinc-200 dark:border-zinc-700">
                    ...
                    >
                        ...
                        <flux:icon
                            name="{{ $showSeo ? 'chevron-up' : 'chevron-down' }}"
                    ...
                    @if ($showSeo)
                        <div class="space-y-4 border-t border-zinc-100 p-4 dark:border-zinc-800">
                            <flux:input
                                ...
                                label="{{ __('Meta title') }}"
                                placeholder="{{ $name ?: __('Category name') }}"
                            />
                            <flux:textarea
                                wire:model="meta_description"
                                label="{{ __('Meta description') }}"
                                placeholder="{{ __('Brief description for search engines...') }}"
                                rows="2"
                            />
                            <flux:input
